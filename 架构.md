...
```
龙虾军团/
├── server.ts                 # 主服务入口：飞书 WebSocket → 消息解析 → Cursor Agent 调度 → 流式回传 → 长消息分片
├── start.ts                  # 启动引导
...
6. **server.ts → 长消息分片**：最终结果超出飞书卡片上限（3800 字符）时，自动拆分为多张卡片发送；复用已有流式卡片承载首段内容，后续分片作为新消息追发
...
- **本地 AI 能力**：Cursor Agent CLI 运行在同一台机器上，通过进程 spawn 通信
