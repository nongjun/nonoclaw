...
export default function GitPanelHeader({
  ...
  const requestPullConfirmation = () => {
    onRequestConfirmation({
      type: 'pull',
      message: `从 ${remoteName} 拉取 ${behindCount} 个提交？`,
      onConfirm: onPull,
    });
  };
  ...
  const requestPushConfirmation = () => {
    onRequestConfirmation({
      type: 'push',
      message: `推送 ${aheadCount} 个提交到 ${remoteName}？`,
      onConfirm: onPush,
    });
  };
  ...
  const requestPublishConfirmation = () => {
    onRequestConfirmation({
      type: 'publish',
      message: `发布分支 "${currentBranch}" 到 ${remoteName}？`,
      onConfirm: onPublish,
    });
  };
              ...
              {remoteStatus?.hasRemote && (
                <span className="flex items-center gap-1 text-xs">
                  ...
                  {remoteStatus.isUpToDate && (
                    <span className="text-muted-foreground" title="已与远程同步">
                      {'\u2713'}
                    </span>
                  )}
                </span>
            ...
            <>
              {!remoteStatus.hasUpstream && (
                <button
                  ...
                  title={`发布分支 "${currentBranch}" 到 ${remoteName}`}
                >
                ...
                <>
                  {behindCount > 0 && (
                    <button
                      ...
                      title={`从 ${remoteName} 拉取 ${behindCount} 个提交`}
                    >
                      <Download className={`h-3 w-3 ${isPulling ? 'animate-pulse' : ''}`} />
                      <span>{isPulling ? '拉取中...' : `拉取 ${behindCount}`}</span>
                    </button>
                  )}
                  ...
                  {aheadCount > 0 && (
                    <button
                      ...
                      title={`推送 ${aheadCount} 个提交到 ${remoteName}`}
                    >
                      <Upload className={`h-3 w-3 ${isPushing ? 'animate-pulse' : ''}`} />
                      <span>{isPushing ? '推送中...' : `推送 ${aheadCount}`}</span>
                    </button>
                  )}
