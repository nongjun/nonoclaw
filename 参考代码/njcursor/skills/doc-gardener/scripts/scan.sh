...
if [ -f "$ARCH_FILE" ] && command -v docker &>/dev/null; then
    PORT_MISMATCH=0
    # 检查几个核心容器的端口
    for container in portal-backend portal-frontend contact-backend scrm_backend moment-backend hosting-backend scrm_archive_backend; do
...
if [ -f "$ARCH_FILE" ]; then
    ...
    MODULE_MISMATCH=0
    for module_dir in 门户系统 联系人 内容中心 会话存档 智能回复建议 朋友圈 企微托管 撩回搭子 老带新 Ai功能 公共模块; do
        if [ ! -d "$PROJECT_ROOT/$module_dir" ]; then
            echo "  ⚠ 架构.md提到但目录不存在: $module_dir/"
            MODULE_MISMATCH=$((MODULE_MISMATCH + 1))
            WARNINGS=$((WARNINGS + 1))
        fi
    done
    # 检查实际存在的模块目录是否在架构.md中提到
    for dir in "$PROJECT_ROOT"/*/; do
        dirname=$(basename "$dir")
        # 跳过非模块目录
        case "$dirname" in
            文档|备份|backup-tools|参考代码|Untitled|.git|node_modules) continue ;;
        esac
