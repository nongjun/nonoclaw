...
4. UI 验证：
   browser_navigate → https://contact.ireborn.com.cn/contacts
...
4. 系统验证（完整）：
   # 健康检查
   curl -s -o /dev/null -w "%{http_code}" https://moment.ireborn.com.cn/api/health
   → 200

   # 接口调用测试
   curl -X DELETE https://moment.ireborn.com.cn/api/moments/batch \
   ...
   # 响应时间
   curl -s -o /dev/null -w "%{time_total}" https://moment.ireborn.com.cn/api/moments/batch
   → 0.15s
...
3. 系统验证（完整）：
   # HTTPS 可达
   curl -s -o /dev/null -w "%{http_code}" https://new-service.ireborn.com.cn
   → 200

   # SSL 证书
   curl -vI https://new-service.ireborn.com.cn 2>&1 | grep "subject:"
   → ✅ 证书正确

   # 反向代理到正确后端
   curl -s https://new-service.ireborn.com.cn/api/health
   → {"status": "ok"}

4. UI 验证（轻量）：
   browser_navigate → https://new-service.ireborn.com.cn
...
5. 系统验证（完整 — 后端为主）：
   curl https://contact.ireborn.com.cn/api/tags
   → 200, [{"id":1,"name":"VIP"}, ...]
...
6. UI 验证（完整 — 前端为主）：
   browser_navigate → https://contact.ireborn.com.cn/tags
