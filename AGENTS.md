...
4. **文档极简主义** — 禁止生成任何临时性文档（变更日志、巡检/审计/测试/完成报告、经验提取独立文件）。Git 是唯一变更记录器，可复用经验写入 Cursor rules，文档只记录代码无法表达的信息（决策原因、业务规则、架构约束）
...
**会话中** → 重要决策、教训、偏好立即写入 memory 文件（禁止"心理笔记"）
**每 12h 自动蒸馏** → distill-chats.ts 提取对话 → _chat-extract.md → Agent 提炼到 MEMORY.md

| 文件 | 职责 |
|------|------|
| `.cursor/memory/YYYY-MM-DD.md` | 每日原始日志，实时写入 |
| `.cursor/MEMORY.md` | 提炼后的长期记忆 |
| `memory.ts` / `memory-tool.ts` | 向量搜索引擎 / Agent CLI 接口 |

**铁律：** "心理笔记"是幻觉，想记住 → 立刻写文件。
...
**飞书卡片限制：** 单卡最多 5 个表格 · JSON ≤ 30KB（约 3500 中文字）· 超限需分片或写文件

**安全红线：** 禁止硬编码密钥 · 禁止日志输出敏感信息 · 禁止暴露内部地址 · 禁止 .env 入 Git · 所有输入必须校验，DB 操作参数化
