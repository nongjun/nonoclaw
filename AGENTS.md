...
本仓库既是开源项目的源码，也同时作为开发实例运行。所有远程部署（服务器上的 nonoclaw）都是实例化部署，其使用反馈反哺开源仓库的迭代。

### 三层文件分类

| 层 | 内容 | Git 状态 | 说明 |
|---|------|---------|------|
| **代码层** | `*.ts`, `*.sh`, `feishu/` 等 | 追踪 | 所有实例共享的代码逻辑 |
| **规则层** | `.cursor/rules/*.mdc`, `skills/`, `commands/`, `agents/` | 追踪 | 共享的 AI 行为 DNA，`git pull` 时同步更新 |
| **实例层** | `.cursor/MEMORY.md`, `IDENTITY.md`, `SOUL.md`, `USER.md` 等 | gitignored | 每个部署的私有数据，不进入公开仓库 |

### 模板与实例的关系

- `templates/.cursor/` — 通用模板（跟随 Git，含占位符的工厂默认值）
- 根目录 `.cursor/*.md` — 运行时实例数据（gitignored，AI 自主读写维护）
- `server.ts` 的 `ensureWorkspace()` 仅在目标文件不存在时从模板复制，**不会覆盖**已有实例文件

### 更新安全

实例执行 `git pull` 时：
- 代码和规则正常更新（共享层的改进惠及所有实例）
- 模板文件更新（但不影响已初始化的实例数据）
- 实例层文件（记忆、身份、用户画像）因 `.gitignore` 不受影响

### 双写原则

改进规则、模板、配置时，必须同时写入两处：

1. **实例文件**（如 `.cursor/rules/soul.mdc`）— 当前实例立即生效
2. **模板文件**（如 `templates/.cursor/rules/soul.mdc`）— 新实例初始化时获得改进

只改实例不改模板 → 新实例享受不到改进。只改模板不改实例 → 当前部署不生效。**双写才是闭环。**

适用范围：`templates/` 中有对应文件的所有规则（`.cursor/rules/*.mdc`）和数据文件（`.cursor/*.md`）。仅存在于实例层的文件（如实例特有的编码规范、业务规则）不需要双写。
...
文档分两层：**代码可表达的用规则**（`.cursor/rules/*.mdc`），**代码无法表达的用文档**（`文档/`）。
